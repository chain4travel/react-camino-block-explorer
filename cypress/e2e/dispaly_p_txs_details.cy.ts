...
describe('Explorer: Latest block list and transaction list in P chain', () => {
    beforeEach(() => {
        cy.entryExplorer()
    ...
    it('verify latest block list and transaction list', () => {
        ...
        cy.wait('@transactions', { timeout: 30000 }).then((intercept) => {
            ...
            cy.get('h5:contains(Input)').its('length').then(res=>{
                console.log(res)
            })
            cy.get('@inputBlock').each(($el, index) => {
                cy.wrap($el)
                    ...
                    .invoke('text')
                    .then((fromAddress) => {
                        expect(fromAddress).to.equal(transactions.inputs[index].output.addresses[0])
                    })
                ...
                cy.wrap($el)
                    ...
                    .then((value) => {
                        ...
                        const amount = roundedToLocaleString(
                            getDisplayAmount(
                                BigNumber(transactions.inputs[index].output.amount).toNumber()
            ...
            cy.get('@outputBlock').each(($el, index) => {
                cy.wrap($el)
                    ...
                    .invoke('text')
                    .then((fromAddress) => {
                        expect(fromAddress).to.equal(transactions.outputs[index].addresses[0])
                    })
