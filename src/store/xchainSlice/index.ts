...
import { fetchXPTransactions, loadNumberOfPXTransactions, loadTotalPXGasFess } from './utils'
        ...
        builder
            ...
            .addCase(fetchXPTransactions.fulfilled, (state, action) => {
                ...
                if (action.payload.type === 'x')
                    state.xTransactions = action.payload.transactions.map(createTransaction)
                else if (action.payload.type === 'p')
                    state.pTransactions = action.payload.transactions.map(createTransaction)
            })
...
export const selectAllXTransactions = (state: RootState) => state.xchain.xTransactions
// Select All P Transactions
export const selectAllPTransactions = (state: RootState) => state.xchain.pTransactions
// Select Loading Status
export const getXPchainStatus = (state: RootState) => state.xchain.loadXPTransactions
export const getXchainError = (state: RootState) => state.xchain.error
// Select ChainOverreview data for x-chain
export const getXchainOverreview = (state: RootState) => state.xchain.XChainOverview
...
export const getPchainOverreview = (state: RootState) => state.xchain.PChainOverview
// Select TimeFrime for p-chain
export const getTimeFramePchain = (state: RootState) => state.xchain.pTimeFrame
// Actions
export const { changetimeFrameXchain, changetimeFramePchain, resetXPChainReducer } =
    xchainSlice.actions
// Reducer
