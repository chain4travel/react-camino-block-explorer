...
export const fetchBlocksTransactions = createAsyncThunk('cchain/fetchBlocks', async () => {
    const response = await axios.get(`${getBaseUrl()}/v2/cblocks?limit=8&limit=8`)
    return response.data
})

export const fetchCBlockDetail = createAsyncThunk('cchain/blockDetail', async (number: number) => {
...
export const fetchTransactionDetails = createAsyncThunk(
    ...
    async (adress: string) => {
        const res = (await axios.get(`${getBaseUrl()}/v2/ctransactions?hash=${adress}`)).data
            .Transactions[0]
        return res
    },
)
