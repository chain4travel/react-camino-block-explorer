...
function SelectedNetwork({ value, networkStatus }: { value: string; networkStatus: string }) {
    return (
        <Box
            sx={{
                display: 'flex',
        ...
        >
            <Chip
                sx={{
                    ...
                    height: '8px',
                    backgroundColor:
                        networkStatus === 'failed' || value === 'Mainnet' ? '#DD5E56' : '#35E9AD',
                }}
            />
            <Box
                style={{
...
export default function NetworkSelect() {
    ...
    const [network, setNetwork] = React.useState(nameOfActiveNetwork(networks, activeNetwork))
    ...
    const handleRemoveCustomNetwork = (id: string) => {
        ...
        const newCustomNetworks = customNetworks.filter(network => network.id !== id)
...
const NewNetwork = () => {
    ...
    const handleDuplicateNetworkId = (NewNetwork: Network, networks: Network[]) => {
        if (networks.find(item => item.id === NewNetwork.id && item.predefined === false)) {
            return true
        }
        return false
    }
                ...
                >
                    <FormControl fullWidth variant="filled" size="medium">
                        ...
                        <TextField
                            ...
                            color="secondary"
                            fullWidth
                            onChange={e => setNewNetwork({ ...NewNetwork, host: e.target.value })}
                        />
                        <TextField
                            id="port"
                            label="Port"
