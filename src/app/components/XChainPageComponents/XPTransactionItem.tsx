        ...
        <>
            ...
            <Grid container item xs={12} md={9} lg={10}>
                <Grid item xs={8} md={6} lg={8}>
                    <AddressLink to={to} value={id} typographyVariant="body1" truncate={true} />
                ...
                >
                    <Chip label={type} size="small" sx={{ minWidth: '50px', height: '20px' }} />
                </Grid>
            </Grid>
        </>
    )
...
const XPTransactionThirdSection = ({ value }: { value: number }) => {
    ...
    return (
        <>
            {isWideScreenDown && (
                <Divider variant="fullWidth" sx={{ width: '100%', marginBottom: '1rem' }} />
            )}
            <Grid container>
                {isWideScreenDown && (
