...
export function getNextPrevTransaction(direction: boolean, transaction?: TransactionInformations) {
    return async function getTx(dispatch: AppDispatch, getState: () => RootState) {
...
export const fetchPrevTransactionDetails = createAsyncThunk(
    ...
    async (infos: TrimmedTransactionDetails) => {
        let url = `${getBaseUrl()}/v2/cblocks?limit=${0}&limit=${50}&blockStart=${
            infos.blockNumber
        }&blockEnd=${NaN}&transactionId=${infos.transactionID}&address=${infos.address}`
